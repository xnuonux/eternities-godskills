...
The compact revision has now had two matched coding comparisons. The first showed lower treatment usage with equal checked functionality; the real maintenance follow-up showed higher treatment input usage, with equal functional checks and a separately recorded baseline scope breach. See [maintenance results](maintenance-pilot/RESULTS.md). Default card adoption remains deferred and this pilot series is closed. Do not rerun the earlier next-step prescription automatically. Keep the useful verified catalog-status implementation separate from the unresolved claim that more guidance improves efficiency; resume source-family refinement next.
