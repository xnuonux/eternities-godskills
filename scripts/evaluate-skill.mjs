...
export async function evaluateSkill({ skillPath, policyPath, receiptPath }) {
  ...
  const [skillText, casesText, policyText] = await Promise.all([
  ...
  const receipt = {
    ...
    evaluationMode: "deterministic-contract-fixture",
    limitation:
      "This receipt proves declared routing, structure, and promotion-policy invariants; live model recall remains a separate fresh-prompt proof.",
    evidence: {
      ...
      candidateSourceCoverage: suite.candidate.sourceCoverage,
    },
    baseline,
    candidate,
    decision,
