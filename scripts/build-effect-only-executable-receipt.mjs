...
const ENTRY = 'scripts/effect-only-v2.mjs';
const BUILDER = 'scripts/build-effect-only-executable-receipt.mjs';
const SOURCES = [ENTRY,'src/effect-intent-v2.mjs','src/intent-contracts.mjs','src/io.mjs','src/routing-contracts.mjs'];
const TESTS = ['tests/effect-intent-v2.test.mjs','tests/effect-only-v2-cli.test.mjs','tests/effect-only-executable-receipt.test.mjs'];
...
async function observedTests(root, mode, sourceCommit) {
  const logPath = `artifacts/effect-only-v2/${mode}.log`;
 ...
 */
export async function buildEffectOnlyExecutableReceipt({repositoryRoot,sourceCommit,review}) {
