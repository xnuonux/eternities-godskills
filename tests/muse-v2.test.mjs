...
test("Muse v2 preserves all four v1 routes and adds bounded visual systems", async () => {
  ...
  assert.deepEqual(synthesis.preservedRoutes, ["visual-forensics", "interface-art-direction", "motion-story", "visual-acceptance"]);
...
test("Muse v2 retains fail-closed boundaries and excludes non-promoted evidence", async () => {
  const [synthesis, cases, clusterSet] = await Promise.all([
    json("syntheses/eternities-muse.v2.json"),
    json("skills/eternities-muse/evals/cases.v2.json"),
    json("clusters/visual-3d-motion.v1.json"),
  ]);
  const nonCandidate = clusterSet.clusters.filter(({ synthesisDecision }) => synthesisDecision !== "candidate");
  assert.deepEqual(synthesis.deferredClusters, nonCandidate.map(({ id }) => id));
  assert.match(synthesis.boundaries.failClosed.join(" "), /rights|shader|render|publication|credential|mutation/i);
...
test("Muse v2 hashes its exact artifacts and declares fixture limits", async () => {
  ...
  assert.equal(receipt.evidence.synthesisSha256, sha256(synthesisText));
  assert.equal(receipt.evidence.artifacts.skill.sha256, sha256(skillText));
  assert.equal(receipt.evidence.artifacts.capabilityContract.sha256, sha256(contractText));
  assert.equal(receipt.evidence.artifacts.routingCard.sha256, sha256(routingText));
  assert.equal(receipt.evidence.artifacts.evaluation.sha256, sha256(casesText));
  assert.equal(receipt.evidence.artifacts.synthesis.sha256, sha256(synthesisText));
  assert.equal(receipt.evidence.artifacts.promotionReceipt.path, "receipts/promotions/eternities-muse-v2.json");
