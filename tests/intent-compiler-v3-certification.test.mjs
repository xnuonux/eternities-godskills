...
test("intent compiler v3 certifies the twenty-one-card arena and prior generalization", async () => {
  ...
  assert.equal(actual.metrics.currentArena.caseCount, 148);
  assert.equal(actual.metrics.currentArena.failCount, 0);
  assert.equal(actual.metrics.currentArena.positiveExactSelectionCount, 89);
